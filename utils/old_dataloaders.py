...
def get_random_patches(img, path_image, bodies_annotations, nb_patches, window_size, prob_empty):
    patches_X = []
    patches_y = []
    paths = []
    shapes = []
    ...
    for i in range(nb_patches):
        if nb_bodies: # si la imagen tiene cuerpos, se extraen 4 patches de un cuerpo aleatorio
            ...
            if len(boxes):
                for b in boxes: 
                    x = (b[0] + (abs(b[2]-b[0])/2)) / window_size
                    y = (b[1] - (abs(b[3]-b[1])/2)) / window_size
                    w = (abs(b[2] - b[0])) / window_size
                    h = (abs(b[3] - b[1])) / window_size
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        self.patches_imgs, self.patches_labels, self.patches_paths, self.patches_shapes = self.extract_patches(self.im_files)
        print("AFTER EXTRACT PATCHES ------------------------------------------------------")
        print("len patches: ", len(self.patches_imgs))
        print("len labels: ", len(self.patches_labels))
        print("patches X shape: ", self.patches_imgs[0].shape)'''
        
        # Leer anotaciones y actualizarlas de los ficheros .cache
...
class HUBDatasetStats():
    ...
    def process_images(self):
        print("entra funcion process_images")
