...
def get_random_patches(img, path_image, bodies_annotations, nb_patches, window_size, transforms, prob_empty):
    ...
    for i in range(nb_patches):
        if nb_bodies: # si la imagen tiene cuerpos, se extraen 4 patches de un cuerpo aleatorio
            ...
            patch_pil = Image.fromarray(patch_pil)
            ...
            if transforms:
                ...
                #tfn = np.array(tfn)
                tfn = tfn.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
                tfn = np.ascontiguousarray(tfn) 
                patches_X.append(tfn)
            # NON AUGMENTED
            else:
                patch = patch_img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
                patch = np.ascontiguousarray(patch)
            ...
            # AUGMENTED
            if transforms:
                patches_X.append(transforms(patch.copy()))
            # NON AUGMENTED
            else:
                patches_X.append(patch.copy() )
            #patches_X.append(patch.copy())
