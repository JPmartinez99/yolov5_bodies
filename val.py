...
def get_data_loaders(opt):
    ...
    train_db    = SeaDronSee(dataset_dir, partition='training', config=opt, transforms=get_transform())
    val_db      = SeaDronSee(dataset_dir, partition='validation',   config=opt, transforms=get_transform())
    test_db     = SeaDronSee(dataset_dir, partition='test',  config=opt, transforms=get_transform())
    ...
    return [torch.utils.data.DataLoader(db, batch_size=1, shuffle=True, num_workers=4, collate_fn=collate_fn) \
                for db in [train_db, val_db, test_db]]
...
def run(
    ...
    iouv = torch.linspace(0.2, 0.95, 16, device=device)  # iou vector for mAP@0.5:0.95
    niou = iouv.numel()
    ...
    if not training:
        ...
        train_loader, val_loader, test_loader = get_data_loaders(opt)
        if task == 'train':
