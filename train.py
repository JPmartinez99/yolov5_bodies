...
from utils.dataloaders import SeaDronSee
...
def get_data_loaders(opt):
    ...
    return [torch.utils.data.DataLoader(db, batch_size=1, shuffle=True, num_workers=4, collate_fn=collate_fn) \
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    print("old img size: ", opt.imgsz)
    print("new img size: ", imgsz)
    ...
    # Trainloader
    train_loader2, dataset = create_dataloader(train_path,
                                              imgsz,
    ...
    '''for i, (imgs2, annotations2, paths2, shapes2) in enumerate(train_loader2):
        if(i==0 or i==1):
            ...
            print("annotations: ", annotations2)
            print("Paths: ", paths2)
            print("Shapes: ", shapes2)
            #print("SSS: ", shapes[0][0][0])
            break
    ...
    '''for i, (imgs3, targets3, paths3, shapes3) in enumerate(train_loader):
        if(i==0 or i==1):
            ...
            print("Paths: ", paths3)
            print("Shapes: ", shapes3)
            print("leeen: ", imgs3[0].shape)
            print("ilen:" , len(imgs3[0]))
            break
    print("len trainloader2: ", len(train_loader))'''

    # Process 0
    if RANK in {-1, 0}:
        val_loader2 = create_dataloader(val_path,
                                       imgsz,
    ...
    model.class_weights = labels_to_class_weights(train_db.labels_bbox, nc).to(device) * nc  # attach class weights
    model.names = names
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if opt.image_weights:
            print("Entra en if opt.image_weights")
            cw = model.class_weights.cpu().numpy() * (1 - maps) ** 2 / nc  # class weights
            iw = labels_to_image_weights(train_db.labels_bbox, nc=nc, class_weights=cw)  # image weights
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Multi-scale
            if opt.multi_scale:
                sz = random.randrange(int(opt.window * 0.5), int(opt.window * 1.5) + gs) // gs * gs  # size
...
def parse_opt(known=False):
    ...
    parser.add_argument('--p',         default=1.0,                               type=float,    help='Percentage db')
    ...
    parser.add_argument('-trunc_img',   default=None,                                type=int,    help='Value wich limits number of training samples')
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {
            ...
            'iou_t': (0, 0.1, 0.7),  # IoU training threshold   # 'iou_t': (0, 0.1, 0.7),
